const taxonomyMaps = document.querySelectorAll('#taxonomy-map, #tag-map, #untagged-map, #uncategorized-map');

if (taxonomyMaps.length) {
  const openBranch = branch => {
    if (!branch) return;
    branch.classList.add('is-open');
    branch.querySelector('.taxonomy-node').setAttribute('aria-expanded', 'true');
    branch.querySelector('.taxonomy-branch-body').setAttribute('aria-hidden', 'false');
  };
  ...
  taxonomyMaps.forEach(taxonomyMap => taxonomyMap.addEventListener('click', event => {
    ...
    if (taxonomyButton) {
      ...
      branch.querySelector('.taxonomy-branch-body').setAttribute('aria-hidden', String(!isOpen));
      return;
    }
