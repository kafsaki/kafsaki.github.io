/**
 * Fluid pixel background (Vue mountable).
 *
 ...
 *
 * initBackground(canvas) starts the engine and returns a cleanup function that
 * removes every listener and releases GPU resources.
 */
...
void main() {
  ...
  const setPointer = (x, y) => {
    pointer = [x / window.innerWidth, 1 - y / window.innerHeight];
    pointerInside = 1;
  };
  ...
  const onPointerDown = (event) => setPointer(event.clientX, event.clientY);
